...
class ApplicationContextTest {
    ...
    void run_isInstance() {
        ...
        appCon.components.put(ApplicationRunnable.class, appRunnable2);

        final var mockListener = mock(BeforeRunListener.class);
        final var componentResolver = ArgumentCaptor.forClass(ComponentResolver.class);
        doNothing().when(mockListener).beforeRun(componentResolver.capture());
        appCon.addBeforeRunListener(mockListener);
        ...
        assertSame(appRunnable2, componentResolver.getValue().resolveComponent(ApplicationRunnable.class));

        final var inOrder = inOrder(mockListener, appRunnable1);
        inOrder.verify(mockListener).beforeRun(any());
        inOrder.verify(appRunnable1).run(any());
    }
    ...
    @Test
    void addBeforeListener() {
        appCon.beforeRunListeners = mock(List.class);
        final var listener = mock(BeforeRunListener.class);
        appCon.addBeforeRunListener(listener);
        verify(appCon.beforeRunListeners).add(listener);
    }
